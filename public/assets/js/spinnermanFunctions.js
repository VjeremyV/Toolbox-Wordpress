export { retrieveData, cleanSpin, get_existing_traduction_files, clean_data, end_process }
...
/**
   ...
   * @returns
   */
async function retrieveData(data, options, messagecontainer) {
    ...
    for (const value of Object.keys(data)) {
      ...
      messagecontainer.innerHTML = `<span class="font-weight-bold m-3">Spin en cours ... ${i}/${Object.keys(data).length}</span>`; 
    ...
    return ret;
     ...
     function cleanSpin(text) {
      ...
      async function get_existing_traduction_files(nomBlog, options){
       ...
       fetch("/get_existing_files/" + nomBlog, {
            ...options,
          })
            .then((res) => {
              return res.json();
            ...
            .then((data) => {
              console.log(data)
              let selectOptions = "<option>Selectionner le fichier de traduction</option>";
       ...
       */
      function end_process(makeCsvFileProgressTag, spiderContainer, clean_spinned_array, nomBlog, options, txtFile = null){
        //on fetch l'url qui créée le fichier csv final avec le contenu du tableau nettoyé (clean_spin_array)
        fetch("/make_csv_file/" + nomBlog.value, {
          ...options,
          ...
          .then((data) => {
            ...
            if(txtFile){
              spiderContainer.innerHTML = `<div class="resultApi">Spinnerman a terminé vos spins ! Téléchargez les : <button class=" btn btn-lg btn-primary mt-3 allWaButton"><a class="text-white" href="/assets/uploads/spins/${data}.csv">lot beem anglais</a></butyon> <button class=" btn btn-lg btn-primary mt-3 allWaButton"><a class="text-white" href="/assets/uploads/listes/${txtFile}" target="_blank">La liste de votre titres traduits</a></butyon> </div> <img class="congrats" src="/assets/img/spinnerman/araignee_souriante.png">`;
            } else {
              spiderContainer.innerHTML = `<div class="resultApi">Spinnerman a terminé vos spins ! Téléchargez les : <button class=" btn btn-lg btn-primary mt-3 allWaButton"><a class="text-white" href="/assets/uploads/spins/${data}.csv">lot beem anglais</a></butyon> <button class=" btn btn-lg btn-primary mt-3 allWaButton"><a class="text-white" href="/assets/uploads/listes/${data}.txt" target="_blank">La liste de votre titres traduits</a></butyon> </div> <img class="congrats" src="/assets/img/spinnerman/araignee_souriante.png">`;
            }
          })
          .catch((err) => {
            console.log("Upload fail", err);
            makeCsvFileProgressTag.innerHTML = `<span class="font-weight-bold alert text-danger m-3"><i class="fa-solid fa-xmark"></i> Une erreur est survenue lors de la création du fichier d'export CSV</span>`;
          });
        }